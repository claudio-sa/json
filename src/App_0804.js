...
function App() {
  ...
  */
  ...
  function montaOArray(umaString) {
    ...
    const array_via_map = array_TEMP.map((obj) => {
      return obj.split('==>')[1]
    })
    ...
    for (var i = 0; i < 5; i += 1) {
      //array_obj.push(JSON.parse(array_via_map[i]))
    ...
    // const teste_string = JSON.stringify(teste, null, 4)
    // try {
    //   fs.writeFileSync('output_jason.txt', teste_string)
    //   console.log('JSON data is saved.')
    // } catch (error) {
    //   console.error(error)
    // }

    // let data = JSON.stringify(array_obj, null, 4)
    // //await writeJsonFile.sync('output_jason.json',data);
    // const fs = require('fs')
    ...
    //console.log('T1====>>>', array_obj[10].message.receivedAt)
    ...
    <div className="container">
      <div className="content">
        <p>
          ...
          {teste3[0].message.direction}
        </p>

        <div>
          {array_obj.map((item, key) => (
            <p> {item.name}xxxxxxxx OK</p>
          ))}
        </div>
        <div>
          {array_obj.map((item, key) => (
            <p>{item.ultimo} xxxx OK ===</p>
          ))}
        </div>
        {/*
